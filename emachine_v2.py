...
class EMachine:
    ...
    def orthogonal_projector(self, l, reshape = True):
        ...
        """
        ground_space = self.ground_space2(l)
    ...
    def parent_hamiltonian(self, l, L):
        ...
        for i in range(1, L - l + 2):
            if i == 1:
                ...
                s = np.kron(np.eye(self.mdim**i), np.kron(p, np.eye(self.mdim**(L - l - i))))
            h += s
        return h
    
    def create_mps(self, l):
        for i in range(l-1):
            if i == 0:
                s = self.A
            s = np.tensordot(s, self.A, axes=(2,1)) #(x1,i1,i2)x(x2,i2,i3) -> (x1,i1,x2,i3)
            s = rearrange(s, 'x1 i1 x2 i3 -> (x1 x2) i1 i3')
        return s

    def ground_space2(self, l):
        mps = self.create_mps(l)
    ...
    def mps_inverse(self, l):
        mps = rearrange(self.create_mps(l), 'x i1 i2 -> x (i1 i2)', i1 = self.dim, i2 = self.dim)
